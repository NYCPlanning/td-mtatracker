...
try:
    ...
    df['Subway']=[int(x) for x in df['subways_total_estimated_ridership']]
    df['Bus']=[int(x) for x in df['buses_total_estimated_ridersip']]
    ...
    df['Subway']=pd.to_numeric(df['subways_total_estimated_ridership'])
    # df['SubwayPct']=[pd.to_numeric(x.strip().replace('%',''))/100 for x in df['Subways: % of Comparable Pre-Pandemic Day']]
    df['SubwayPct']=pd.to_numeric(df['subways_of_comparable_pre_pandemic_day'])
    df['SubwayPrior']=df['Subway']/df['SubwayPct']
    df['Bus']=pd.to_numeric(df['buses_total_estimated_ridersip'])
    # df['BusPct']=[pd.to_numeric(x.strip().replace('%',''))/100 for x in df['Buses: % of Comparable Pre-Pandemic Day']]
    df['BusPct']=pd.to_numeric(df['buses_of_comparable_pre_pandemic_day'])
    df['BusPrior']=df['Bus']/df['BusPct']
    df['LIRR']=pd.to_numeric(df['lirr_total_estimated_ridership'])
    ...
    df['LIRRPct']=pd.to_numeric(df['lirr_of_comparable_pre_pandemic_day'])
    df['LIRRPrior']=df['LIRR']/df['LIRRPct']
    df['MNR']=pd.to_numeric(df['metro_north_total_estimated_ridership'],errors='coerce')
    ...
    df['MNRPct']=pd.to_numeric(df['metro_north_of_comparable_pre_pandemic_day'])
    df['MNRPrior']=df['MNR']/df['MNRPct']
    df['AAR']=pd.to_numeric(df['access_a_ride_total_scheduled_trips'])
    # df['AARPct']=[pd.to_numeric(x.strip().replace('%',''))/100 for x in df['Access-A-Ride: % of Comprable Pre-Pandemic Day']]
    df['AARPct']=pd.to_numeric(df['access_a_ride_of_comparable_pre_pandemic_day'])
    df['AARPrior']=df['AAR']/df['AARPct']
    df['BT']=pd.to_numeric(df['bridges_and_tunnels_total_traffic'])
    # df['BTPct']=[pd.to_numeric(x.strip().replace('%',''))/100 for x in df['Bridges and Tunnels: % of Comparable Pre-Pandemic Day']]
    df['BTPct']=pd.to_numeric(df['bridges_and_tunnels_of_comparable_pre_pandemic_day'])
    ...
    df=df[['Date','DateRange','Subway','Bus','Long Island Rail Road',
           'Metro-North Railroad','Access-A-Ride','Bridges and Tunnels']].sort_values(['Date']).reset_index(drop=True)
    ...
    fig.update_layout(
        template='plotly_white',
        title={'text':'<b>MTA Estimated Weekly Ridership and Traffic as Percentage of Pre-Pandemic Ridership (Source: '+"</b><a href='https://data.ny.gov/Transportation/MTA-Daily-Ridership-Data-Beginning-2020/vxuj-8kew/about_data'>MTA</a>"+'<b>)</b>',
               'font_size':20,
               'x':0.5,
               'xanchor':'center',
               'y':0.98,
    ...
    fig.update_layout(
        template='plotly_white',
        title={'text':'<b>MTA Estimated Ridership and Traffic as Percentage of Pre-Pandemic Level</b><br>(7-Day Moving Average)',
               'font_size':20,
               'x':0.5,
               'xanchor':'center',
               'y':0.95,
    ...
    #                    yref='paper',
    #                    yshift=-100)
    fig.add_annotation(text = 'Data Source: <a href="https://data.ny.gov/Transportation/MTA-Daily-Ridership-Data-Beginning-2020/vxuj-8kew/about_data" target="blank">Metropolitan Transportation Authority</a>',
                       font_size=14,
                       showarrow=False,
                       x=1,
                       xanchor='right',
